...
def load_data():
    try:
        ...
        if not df.empty:
            ...
            # 3. [에러 방지] 0.5 감점을 위해 float(실수) 타입으로 변환
...
with tab1:
    ...
    with st.form("add_new_form", clear_on_submit=True):
        # [수정 1] 사이드바에서 선택한 분야를 기본값으로 가져오기
        options = ["피지컬 AI", "데이터 분석", "영단어", "직접 입력"]
        
        # '전체'일 때는 0번(피지컬 AI), 아니면 선택한 분야의 인덱스를 찾습니다.
        default_idx = 0
        if selected_subject in options:
            default_idx = options.index(selected_subject)
        
        new_subj = st.selectbox("분야 선택", options, index=default_idx)
        ...
        if st.form_submit_button("저장"):
            if word and defn:
                # [수정 2] Level을 0이 아닌 0.0(실수형)으로 넣어야 에러가 안 납니다!
                new_row = {
                    "분야": new_subj, 
                    "용어": word, 
                    "정의": defn, 
                    "Level": 0.0,  # <--- 이 부분이 핵심 (0 -> 0.0)
